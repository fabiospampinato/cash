...
z.on=function(a,b,c,d){var f=this;if(!v(a)){for(var k in a)this.on(k,b,a[k]);return this}x(b)&&(c=b,b=!1);B(I(a),function(a){a=V(a);var k=a[0],H=a[1];f.each(function(a,f){a=function pa(a){if(!a.namespace||U(H,a.namespace.split("."))){var n=f;if(b)for(n=a.target;!D(n,b);){if(n===f)return;n=n.parentNode;if(!n)return}a.namespace=a.namespace||"";n=c.call(n,a,a.data);d&&W(f,k,H,pa);!1===n&&(a.preventDefault(),a.stopPropagation())}};a.guid=c.guid=c.guid||C++;oa(f,k,H,a)})});return this};
...
z.contents=function(){var a=[];this.each(function(b,c){p.apply(a,"IFRAME"===c.tagName?[c.contentDocument]:c.childNodes)});return y(a.length&&K(a))};z.find=function(a){for(var b=[],c=0,d=this.length;c<d;c++){var f=t(a,this[c]);f.length&&p.apply(b,f)}return y(b.length&&K(b))};z.has=function(a){var b=v(a)?function(b,d){return!!t(a,d).length}:function(b,d){return d.contains(a)};return this.filter(b)};
